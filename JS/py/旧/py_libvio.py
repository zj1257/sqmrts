...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeVideoContent(self):
		# https://www.libvio.me
        rsp = self.fetch("https://tv.libvio.cc")
        ...
        for a in aList:
            ...
            sid = a.xpath("./@href")[0]
            sid = sid.replace(".html", "").replace("/", "").replace("detail", "").replace("_", "")
            videos.append({
                "vod_id": sid,
                "vod_name": name,
                "vod_pic": pic,
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        url = 'https://tv.libvio.cc/type{0}{1}.html'.format(self.pathsign,params)
        print(url)
        rsp = self.fetch(url)
    ...
    def detailContent(self, array):
        tid = array[0]
        url = 'https://tv.libvio.cc/detail{0}{1}.html'.format(self.pathsign,tid)
        rsp = self.fetch(url)
        if rsp.status_code==404:
            self.pathsign="/"
            url = 'https://tv.libvio.cc/detail{0}{1}.html'.format(self.pathsign,tid)
        ...
        for vl in vodList:
            ...
            for tA in aList:
                if tA.xpath('./text()')[0]!="合集":
                    href = tA.xpath('./@href')[0]
                    name = tA.xpath('./text()')[0]
                    tId = href.replace(".html", "").replace("/", "").replace("play", "").replace("_", "")
    ...
    def searchContent(self, key, quick):
        url = 'https://tv.libvio.cc/index.php/ajax/suggest?mid=1&wd={0}'.format(key)
        # getHeader()
    ...
    def playerContent(self, flag, id, vipFlags):
        result = {}
        # https://tv.libvio.cc/play/714889434-1-1.html
        url = 'https://tv.libvio.cc/play{0}{1}.html'.format(self.pathsign,id)
        rsp = self.fetch(url)
        root = self.html(self.cleanText(rsp.text))
        ...
        for script in scripts:
            if (script.startswith("var player_")):
                ...
                break;
        nid = str(jo['nid'])
        scriptUrl = 'https://tv.libvio.cc/static/player/{0}.js'.format(jo['from'])
