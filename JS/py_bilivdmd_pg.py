...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def gethomeVideoContent(self,key,pg):
        url = 'https://api.bilibili.com/x/web-interface/search/type?search_type=media_bangumi&keyword={0}&page={1}'.format(key,pg)  # 番剧搜索
        if len(self.cookies) <= 0:
            self.getCookie()
        ...
        if rs['numPages']>rs['page']:
            pass
        return result
        
    def homeVideoContent(self):
        key = '奥特曼+中配'
        return self.gethomeVideoContent(key,1)
